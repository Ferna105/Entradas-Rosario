...
export function EventCard({ event, variant = "default" }: EventCardProps) {
  ...
  return (
    <div className="relative">
      ...
      {/* Heart button — fuera del Link para evitar navegación al hacer click */}
      <button
        type="button"
        onClick={handleHeartClick}
        className={[
          "absolute right-3 top-3 z-10 flex h-9 w-9 items-center justify-center rounded-full bg-black/50 backdrop-blur-md transition-colors",
          favorited ? "text-danger" : "text-text-secondary hover:text-danger",
        ].join(" ")}
        aria-label={favorited ? "Quitar de favoritos" : "Guardar evento"}
      >
